...
- `acl` ((#acl)) - This object allows a number of sub-keys to be set which
  ...
  - `tokens` ((#acl_tokens)) - This object holds all of the configured
    ...
    - `root` ((#acl_tokens_root)) - Only used for servers in the [`primary_datacenter`](#primary_datacenter).
      ...
      The `root` token is only installed when a server acquires cluster
      leadership. If you would like to install or change the `acl_root_token`,
      set the new value for `root` in the configuration for all servers. Once
      this is done, restart the current leader to force a leader election. If
      the `root` token is not supplied, then the servers do not create a
      root token. When you provide a value, it should be a UUID. To maintain
    ...
    - `agent_root` ((#acl_tokens_agent_root)) - Used to access [agent endpoints](/api/agent)
...
- `acl_agent_root_token` ((#acl_agent_root_token_legacy)) - **Deprecated
  in Consul 1.4.0. See the [`acl.tokens.agent_root`](#acl_tokens_agent_root)
...
- `acl_root_token` ((#acl_root_token_legacy)) - **Deprecated in Consul
  1.4.0. See the [`acl.tokens.root`](#acl_tokens_root) field instead.** Only
  ...
  The `acl_root_token` is only installed when a server acquires cluster leadership. If
  you would like to install or change the `acl_root_token`, set the new value for `acl_root_token`
  in the configuration for all servers. Once this is done, restart the current leader to force a
  leader election. If the `acl_root_token` is not supplied, then the servers do not create a root
